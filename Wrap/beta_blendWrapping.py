...
INPUT_WRAP_FILE = 'G:\\results\perFrame\\beta_blendWrap_5k.wrap'
OUTPUT_WRAP_FILE = 'G:\\results\perFrame\\beta_blendWrap_5k_Temp.wrap'
...
for i in range(START_FRAME, END_FRAME+1, FRAME_STEP):
    ...
    if i == NEUTRAL_FRAME:
        ...
        # copyString = "copy %s %s" % (NEUTRAL, (IN_MESH % NEUTRAL_FRAME) )
        # os.system(copyString)
        copyfile(NEUTRAL, IN_MESH % NEUTRAL_FRAME)
        continue
    else:
    ...
    cmd = "C:\kyleBathStuff\Wrap3.4.2_Portable\Wrap3.4.2\Wrap3Cmd.exe compute %s" % OUTPUT_WRAP_FILE
    os.system(cmd)
